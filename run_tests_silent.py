...
def main() -> int:
    ...
    parser.add_argument(
        "--show-output",
        action="store_true",
        help="Show printed test output live. By default, pytest captures output.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Deprecated compatibility flag. Output is captured by default; "
            "this option is now accepted but no longer required."
        ),
    )
    ...
    print(f"Capture:    {'disabled/live output (-s)' if args.show_output else 'enabled (--capture=fd)'}")
    print()
